...
Bilder unter 10.000px * 10.000px

*/

public class ConvertHarrisSavePlugin_ extends ImagePlus implements PlugIn {
...
static File dir;
	...
	String showChoice() {
		// display dialog , return false if cancelled or on error.
		int index = 0;
		String[] projektArray = { "VORAN", "AKHWA", "OSCAR"};
		String choice = "";
	...
	void exportCropImages(String ImageToCrop, int CountImages, String Maske) {
		...
		//saveData(String.valueOf(entrys));
		
		int imagecounter = 0;
		
		String[] correct_numbers = {"4", "3", "2", "1", "8", "7", "6", "5", "12", "11", "10", "9"};
		
		exportImages = new String[12];
		...
		//	for(int k=6; k==0; k-=2){
				...
				int calc = i*8+k;
				int bigcalc = calc + 1;
				String x1y1 = good_coords_array[calc]; 
				String x2y1 = good_coords_array[calc + 8];
				String x1y2 = good_coords_array[bigcalc]; 
				String x2y2 = good_coords_array[bigcalc + 8];
