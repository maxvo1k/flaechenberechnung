...
public class ColorThresholderExtended extends PlugInFrame implements PlugIn, Measurements,
 ActionListener, AdjustmentListener, FocusListener, ItemListener, Runnable{
	...
	private static final Font DEFAULT_FONT = font12;
	...
	byte[] drawfillMask;
	byte[][] temp_fillMaskIP_Stack;
	byte[][] fillMaskIP_Stack;
	int global_slices = 0;
	int global_counter = 0;
	int[] pixel_array;
	
	boolean stack_action = false;

	public ColorThresholderExtended() {
	...
	public static void scale(final Component component) {
		...
		final float scale = (float)guiScale;
		if (scale==1f)
			return;
	...
	boolean setup(ImagePlus imp) {
		...
		if ((id!=previousImageID)||(slice!=previousSlice)||(flag) ) {
			...
			//New
			temp_fillMaskIP_Stack = new byte[numSlices+1][];
			pixel_array = new int[numSlices+1];
	...
	public void setIntValues(Integer[] values){
		...
		adjustMinHue(values[0]);
		adjustMaxHue(values[1]);
		adjustMinSat(values[2]);
		adjustMaxSat(values[3]);
		adjustMinBri(values[4]);
		adjustMaxBri(values[5]);
	...
	public void setBoolValues(Boolean[] values){
		bandPassH.setState(values[0]);
		bandPassS.setState(values[1]);
		bandPassB.setState(values[2]);
		darkBackground.setState(values[3]);
	...
	public void updateAfterSetting(ImagePlus imp) {
		updateScrollBars();
		updateLabels();
		updatePlot();
		reset(imp);
	...
	void apply(ImagePlus imp) {
		...
		if(applyingStack) {
			...
			pixel_array[global_slices] = global_counter;
			System.out.println("pixel_counter: " + String.valueOf(global_counter));
			global_counter = 0;
			
			System.out.println("Global-Slizes: " + String.valueOf(global_slices));
			
			if(fillMask != null) {
				temp_fillMaskIP_Stack[global_slices] = fillMask;
				System.out.println("fillMask NICHT null");
			} else {
				System.out.println("fillMask Null!");
			}
			if(temp_fillMaskIP_Stack[global_slices] != null) {
				System.out.println("fillMaskIP_Stack NICHT null");
			} else {
				System.out.println("fillMaskIP_Stack Null!");
			}

			saveData(fillMask, String.valueOf(global_slices) + "_Slice.bin", imp);
			
		}
		
	}
	
	void saveData(byte[] mask, String filename, ImagePlus imp_save) {
		
		String currentPath = getDirectory(imp_save);
		if(currentPath != null){
	...
	public void drawBinary(byte[] drawBinaryMask, ImagePlus imp){
		//ImagePlus imp = WindowManager.getCurrentImage();
		ImageProcessor ip = imp.getProcessor();
		if (ip==null) return;
		int[] pixels = (int[])ip.getPixels();
		...
		int bcolor = Prefs.blackBackground?0xff000000:0xffffffff;
		//for (int i=0; i<numPixels; i++) {
		if(drawBinaryMask != null && numPixels == drawBinaryMask.length) {
			for (int i=0; i<numPixels; i++) {
				if (drawBinaryMask[i]!=0)
					pixels[i] = fcolor; //fill
				else
					pixels[i]= bcolor; //keep
			}
		} else {
			//Fehler, alles füllen, damit kein weiterer Fehler entsteht WARUM????
			for (int i=0; i<numPixels; i++) {
				pixels[i] = fcolor; //fill
			}
	...
	public void drawBinaryStack(byte[] drawBinaryMask, ImagePlus imp, int slice){
		//Stack
		imp.setSlice(slice);
	...
	byte[] getdrawfillMaskStack(int slice){
		//Stack
		return fillMaskIP_Stack[slice];
	}
	
	byte[][] getAlltheMasks(){
		//Stack
		fillMaskIP_Stack = temp_fillMaskIP_Stack;
		return fillMaskIP_Stack;
	}
	
	boolean getStackAction() {
		return stack_action;
	}
	...
	Boolean[] getBoolValues(){
		...
		result[0] = bandPassH.getState();
		result[1] = bandPassS.getState();
		result[2] = bandPassB.getState();
		result[3] = darkBackground.getState();
		return result;
	}
